  ...
  return (
    ...
    <div className=''>
      <div className='sticky top-0 z-50 flex justify-between bg-zinc-300 px-4 py-2 shadow-sm border-2 border-cyan-600'>
        ...
        <div className='flex-grow'>
          <form className='flex items-center space-x-2 border border-cyan-600 rounded-lg
            ...
            <input className='bg-transparent outline-none text-cyan-600' type='text' placeholder='Search by Title' onChange={(e) => setQuery(e.target.value)} />
      ...
      <div className='relative mr-60 ml-60'>
        <ul>
          {papers.filter((paper) =>
            paper.data.title.toLowerCase().includes(query)
          ).map(paper => (
            <Post
              id={paper.id}
